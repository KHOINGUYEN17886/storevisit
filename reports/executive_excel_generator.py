...
class ExecutiveExcelGenerator:
    """
    Generates professional 5-Tab Executive Excel Dashboard (.xlsx)
    Brand Standard: An Phước Navy (#1B2A4A), Red Accent (#C41E3A), Gold (#D4AF37)
    """
    def __init__(self):
    ...
    def _build_summary_tab(self, ws, data):
        ...
        kpi_headers = ["Lượt kiểm tra", "Điểm Sức khỏe Trung bình", "Cửa hàng Đạt / Tốt", "Cửa hàng Chưa Đạt", "Lỗi Khẩn cấp / Cao", "Khảo sát Đối thủ"]
        kpi_values = [
            kpis.get("total_visited", 0),
        ...
        for i, (h, v) in enumerate(zip(kpi_headers, kpi_values), start=2):
            ...
            cell_v.font = Font(name="Calibri", size=14, bold=True, color="1B2A4A")
        ...
        ws.cell(row=11, column=2, value="Hạng mục vi phạm").fill = self.navy_fill
        ws.cell(row=11, column=2).font = self.header_font
        ws.cell(row=11, column=3, value="Số lần phát sinh").fill = self.navy_fill
        ws.cell(row=11, column=3).font = self.header_font
        
        top_issues = data.get("top_systemic_issues", [])
        for idx, issue in enumerate(top_issues, start=12):
            c1 = ws.cell(row=idx, column=2, value=issue.get("label", ""))
            c2 = ws.cell(row=idx, column=3, value=issue.get("count", 0))
    ...
    def _build_leaderboard_tab(self, ws, data):
        ...
        headers = ["Hạng", "ASM Phụ trách", "Cửa hàng Phụ trách", "Đã Kiểm tra", "% Tỷ lệ Phủ", "Điểm Sức khỏe TB", "Vi phạm Nghiêm trọng", "% Tỷ lệ Đạt"]
    ...
    def _build_survey_tab(self, ws, data):
        ...
        for r, surv in enumerate(surveys, start=5):
            ...
            ws.cell(row=r, column=6, value=surv.get("timestamp", ""))
